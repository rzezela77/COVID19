...
**inserir figura: I'm not epidemiologist but ...**

## 1.0 Introdução

Não dá para ficar indiferente perante a essa pandemia que assola o mundo neste momento tão negro. Vamos aplicar o nosso saber para realizar análises de dados usando **R Shiny** desenvolvendo *Dashboard Reporting* de forma voluntária.
...
### 2.1 Valores Totais dos Casos

Vamos criar um *dataframe* para armazenar os valores totais dos casos *confirmado*, *recuperados*, *mortos* e *activos*. O que facilitará a obtenção de percentagens para cada caso.
*serão usados para o InfoCard no Shiny app*
```{r}
...
Vamos mostrar os casos atraves do mapa permitindo visualizar todos os paises e os respectivos casos de forma interactiva. Será possivel visualizar o número total de casos *confirmado*, *recuperados*, *mortos* e *activos*.

Podemos seguir alguns passos:  
1. obter o *mapdata* para averiguar a natureza de dados e para facilitar determinar que variáveis serão usados conforme os dados que dispomos;  
2. criar um *dataframe* para mostrar as variáveis que pretendemos visualizar;  
3. criar o mapa fazendo o *join* com o *dataframe* criado para o efeito.  
...
Como podem observar pelo *output* podemos fazer o *join* entre as tabelas usando a variável *code* ou *name* referindo-se ao nome do país.
...
#### 3.2.1 Criar Funções
Vamos criar funções para mostrar as visualizações. As vantagens de criar funções são enormes e inquestionáveis, desde a manutenção do módulo/projecto e a sua repetitividade. Portanto, no presente trabalho vou colocar aqui a disposição porque normalmente desenvolvo em um ficheiro à parte, por exemplo *getFunctions.R*, conforme as *boas prácticas* . Mas para não vos remeter ao meu *github* para ver o código vou disponobiliza-lo aqui.
...
# New Cases per Day
hc_plot_NewCases(data = NewCases_tbl, countryName = 'United States of America', cumulative = FALSE)

# Cumulative Cases per Day
hc_plot_NewCases(data = NewCases_tbl, countryName = 'United States of America', cumulative = TRUE)
